...
def report(
    ...
    min_missing_share: float = typer.Option(0.1, help="Порог доли пропусков (от 0.0 до 1.0), выше которого колонка считается проблемной."),
    ...
    if not (0.0 <= min_missing_share <= 1.0):
        raise typer.BadParameter("--min-missing-share должен быть в диапазоне [0.0, 1.0]")
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Постоянные колонки: **{quality_flags['has_constant_columns']}**\n")
        ...
        if problematic_missing_cols:
            ...
            f.write("## Проблемные колонки по пропускам\n\n")
            f.write(f"Нет колонок с долей пропусков выше {min_missing_share:.1%}.\n\n")
        ...
        f.write(f"Для каждой категориальной колонки показано **top-{top_k_categories}** значений.\n\n")
        if not top_cats:
        ...
        f.write(f"Показано до **{max_hist_columns}** числовых колонок.\n")
