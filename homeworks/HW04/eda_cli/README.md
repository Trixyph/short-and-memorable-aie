...
К существующему CLI-приложению для EDA добавлен **HTTP-сервис на FastAPI** с эндпоинтами `/health`, `/quality`, `/quality-from-csv` и новым эндпоинтом `/quality-flags-from-csv`.  
...
- используются функции из HW03: `summarize_dataset`, `missing_table`, `compute_quality_flags`.
...
### 5. `POST /quality-flags-from-csv` – полный набор флагов качества (НОВЫЙ)
...
- вызывает функции из `eda_cli.core`:
  ...
  - `missing_table`,
  - `compute_quality_flags`;
- возвращает полный набор флагов качества, включая числовые значения и все эвристики из HW03.
...
Флаги включают эвристики из HW03:

- `has_constant_columns` - есть ли колонки с одним уникальным значением;
- `has_many_zero_values` - есть ли числовые колонки с долей нулей > 90%;
- `quality_score` - интегральная оценка качества (0.0-1.0);
- `max_missing_share` - максимальная доля пропусков;
- `too_many_column` - количество столбцов > 100;
- `too_few_rows` - количество строк < 100;
...
3. Проверить работу HTTP-сервиса (`uv run uvicorn ...` и `/health` и `/quality`/`/quality-from-csv` через `/docs` или HTTP-клиент).